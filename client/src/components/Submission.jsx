...
const Submission = () => {
 const StudentName= localStorage.getItem("name")
  ...
  const { score, time } = useContext(ContextData);
      ...
      {showImage ? ( 
        ...
        >
          ...
          <p className="text-lg text-gray-600 mb-4">
            {StudentName}, your score is: <strong className="text-blue-600">{score}</strong>
          </p>
          <p className="text-lg text-gray-600">
            Your total time is: <strong className="text-green-600">{formatTime(time)}</strong>
          </p>
          <NavLink to="/home">
            <button
